# GPT-4o FinanceBench Experiment Report (v4)

## What Changed Since v3

- **Re-evaluated all experiments** with an improved judge, producing significantly higher accuracy across the board (e.g., single-turn QA moved from 20.67% to 28.67%, best default-agent config from 54.00% to 66.00%).
- **Removed Exp 2 (naive search single-turn)** — no judged output available in current results.
- **New Exp 9: Added reranker** (`BAAI/bge-reranker-v2-m3`) on top of BGE-large + calculator → 66.67%.
- **New Exp 10: ReAct agent** (`agent_name: react`) replaces the default agent, achieving the new best: **70.00%**.
...
Replacing the default embedding model with `BAAI/bge-large-en-v1.5` yielded a modest improvement (64.00% → 65.33%), with the incorrect rate dropping from 20.00% to 18.67%. The gains are smaller than in v3's evaluation (which showed +3.3pp), suggesting the improved judge narrows the apparent delta.
...
2. **Multi-query retrieval** — the detailed error analysis (see reranker report) shows retrieval failure remains the #1 failure mode (34% of failures). Having the agent issue multiple diverse queries per search could help.
...
4. **Stronger models** — GPT-5 experiments are underway in the results folder; comparing GPT-4o vs GPT-5 with the same configs will isolate the model capability effect.
